...
var findUser = function (req, res, callBack) {
    mongoClient.connect(mogoUrl, function (err, db) {
        var cursor = db.collection('user').find({ "email": req.query.email }).toArray(function (err, doc) {
            if (err) {
...
var getUser = function (req, res, email, routeName = null) {
    mongoClient.connect(mogoUrl, function (err, db) {
        var cursor = db.collection('user').find({ "email": email }).toArray(function (err, doc) {
            if (err) {
                res.send({ status: 'failed', msg: '服务器错误稍后重试' });
                db.close();
            } else if (!doc || doc.length == 0) {
                        ...
                        db.collection('course').find({}).sort({ day: 1 }).toArray(function (err, doc) {
                            if (doc && doc.length > 0) {
                                user.progress = user.progress.map(function (child, index) {
                                    return {
                                        ...child,
                                        unLocked: new Date() >= new Date(doc[index].time)
                                    }
                                });
                            }
...
var addUser = function (req, res, info = null) {
    mongoClient.connect(mogoUrl, function (err, db) {
        var cursor = db.collection('user').find({ "email": req.query.email }).toArray(function (err, doc) {
            if (err) {
                res.send({ status: 'failed', msg: '服务器错误稍后重试' });
                ...
                if (info) {
                    db.collection('user').updateOne({ "email": req.query.email }, {
                        $set: {
                            "openId": info.openId,
                            "name": info.name,
                            "avator": info.avator,
                        ...
                        res.cookie('hasAvator', info.avator ? 1 : 0, { maxAge: 60 * 24 * 60 * 60 * 1000 })
                        res.send({ status: 'success', msg: '成功添加' });
                        db.close();
                    });
                }
    ...
    mongoClient.connect(mogoUrl, function (err, db) {
        db.collection('user').find({}).toArray(function (err, doc) {
            if (doc && doc.length > 0) {
                ...
                res.render('ranking', { readGroup, inGroup })
            }
            else {
                res.send({ status: 'failed', msg: '服务器错误稍后重试' });
                db.close();
